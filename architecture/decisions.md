...
- help text, examples, and defaults are optimized around `occams-beard run`
...
2. remove the `occams-beard/` directory from the repo
...
DESCRIPTION:
Repository documentation and user-facing UI text still used the previous project name after the repository had already moved to `occams-beard`, which created avoidable branding drift and made the review surface feel inconsistent.

IMPACT:
Reviewers and operators could see different project names depending on whether they were reading docs, viewing the web UI, or opening sample report artifacts.
...
ROOT CAUSE:
The repository move and broader documentation cleanup happened separately from earlier UI and report copy, leaving stale product naming in multiple surfaces.

RECOMMENDED RESOLUTION:
Update human-facing documentation, HTML templates, launcher copy, and report artifacts to use one consistent project name.
...
DESCRIPTION:
The root README had accumulated duplicated interface guidance, stale repository-path examples, and an incomplete documentation map after the project was moved to the repository root.

IMPACT:
Readers had to reconcile repeated sections manually, and installation or verification snippets could point at a no-longer-current directory layout.
...
ROOT CAUSE:
Documentation updates were additive across several implementation passes, but no consolidation pass had normalized the repository layout and reader flow end to end.

RECOMMENDED RESOLUTION:
Consolidate the README around current interfaces and repo structure, update path examples to `occams-beard`, and add a clearer documentation map.
