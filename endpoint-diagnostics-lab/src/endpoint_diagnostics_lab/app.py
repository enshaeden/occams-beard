...
def create_app(config: dict | None = None) -> Flask:
    ...
    def download_json(run_id: str):
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=endpoint-diagnostics-{run_id}.json"
            },
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="endpoint-diagnostics-lab-web",
        description="Run the local Endpoint Diagnostics Lab web interface.",
    )
