...
def evaluate_selected_findings(
    ...
    *,
    ...
    if "time" in enabled:
        findings.extend(_evaluate_time_state(facts, enabled_checks=enabled))
    ...
    if time_state.timezone_offset_consistent is False:
        findings.append(
            Finding(
                ...
                evidence=_timezone_consistency_evidence(time_state),
                probable_cause=(
    ...
    if skew_check is not None and skew_check.status not in {"not_run", "checked"}:
        findings.append(
            Finding(
                ...
                evidence=_inconclusive_time_evidence(time_state),
                probable_cause=(
    ...
    *,
    ...
    evidence = [
        ...
        (
            ...
            f"{_format_utc_offset(time_state.utc_offset_minutes)}."
...
def _timezone_consistency_evidence(time_state) -> list[str]:
    evidence = [
        f"Local time snapshot is {time_state.local_time_iso}.",
        (
...
def _inconclusive_time_evidence(time_state) -> list[str]:
    evidence = [
        f"Local time snapshot is {time_state.local_time_iso}.",
        (
            f"Timezone identifier is {time_state.timezone_identifier}."
